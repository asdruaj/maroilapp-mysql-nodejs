...
class DataTable{
  ...
  parse(){
    ...
    renderRows(){
      ...
      for(i = pointer; i < limit; i++){
        ...
        data += `<td class="table-checkbox">
                    <input type="checkbox" id="checkbox${values[0]}" class="datatable-checkbox" data-id="${values[0]}" name="id[${values[0].trim()}]" value="${values[0].trim()}" ${checked? "checked":""} </>
        </td><td><form method="get" action="/reports/edit/${Number(values[0])}"><button class="bEdit" type="submit"><i class="material-icons edit">edit</i></button></form></td>`;
        
        values.forEach(cell =>{
          data += `<td><label for="checkbox${values[0]}">${cell}</label></td>`;
        });
        this.element.querySelector('tbody').innerHTML += `<tr class="row">${data}</tr>`;
    ...
    renderPagesButtons(){
      ...
      }else if(Math.min(superiorLimit + missingButtons, this.pagination.noPages) !== this.pagination.noPages){
