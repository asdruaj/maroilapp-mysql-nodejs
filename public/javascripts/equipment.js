...
tbody2.onclick = function (e) {
  ...
  for (let row of failureRows) {
    // `for...of` loops through the NodeList
    failureCells = row.getElementsByTagName("td");
    if (
      nData[0] == failureCells[2].textContent &&
      nData[2] == failureCells[3].textContent
    ) {
      row.style.display = ""; // shows this row
    } else {
      row.style.display = "none"; // hides this row
...
btnEditEquipment.forEach((el) => {
  ...
  el.addEventListener("click", () => {
    ...
    document.getElementById(
      "modalHeader"
    ).textContent = `Editando Equipo N° ${equipmentId} . . .`;
  });
});
...
btnEditSpecialty.forEach((el) => {
  ...
  el.addEventListener("click", () => {
    ...
    document.getElementById(
      "modalHeader"
    ).textContent = `Editando Tipo de Falla N° ${specialtyId} . . .`;
  });
});
...
btnEditFailure.forEach((el) => {
  ...
  el.addEventListener("click", () => {
    ...
    document.getElementById(
      "modalHeader"
    ).textContent = `Editando Falla N° ${failureId} . . .`;
  });
});
