module.exports={
  ...
  insertUser: async function (cn, data, func) {
    cn.query("INSERT INTO `tbl-users` (`name`, `username`, `password`, `role`) VALUES (?, ?, ?, ?)",[data.name, data.username, data.password, data.role], func);
  },
  ...
  returnIdData: async function (cn,id,func){
   ...
   updateUser: async function (cn, data, func) {
    cn.query("UPDATE `tbl-users` SET `name`=?, `username`=?, `password`=?, `role`=? WHERE `id`=?",[data.name, data.username, data.password, data.role, data.id], func);
  },
}